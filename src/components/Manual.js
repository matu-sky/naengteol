...
function Manual({ navigate, setIngredients, setRecipes, setWeeklyPlan, apiKey }) {
  ...
  return (
    <div className="screen">
      ...
      {items.length > 0 && (
        ...
        >
          {loading ? '🍳 레시피 찾는 중...' : filter === '일주일식단' ? '📅 일주일 식단 추천받기' : '🍳 레시피 추천받기 (' + items.length + '개 재료)'}
        </button>
      )}
    </div>
  );
